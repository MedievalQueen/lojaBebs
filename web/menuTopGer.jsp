<%-- 
    Document   : menuTopGer
    Created on : May 13, 2015, 3:39:47 PM
    Author     : Ina
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>

     <ul class="menu">
        <li><a href="redir_home.jsp" class="nav"> Home </a></li>
        <li class="divider"></li>
        <li><a href="redir_loja.jsp" class="nav">Nossa Loja</a></li>
        <li class="divider"></li>
        <li><a href="servletBuscaProd" formmethod="post" class="nav">Produtos</a></li>
        <li class="divider"></li>
        <li><a href="redir_busca.jsp" class="nav">Busca Clientes</a></li>
        <li class="divider"></li>
         <li><a href="redir_cadastroCateg.jsp" class="nav">Cadastro de Categoria</a></li>
        <li class="divider"></li>
         <li><a href="servletListaCat" formmethod="post" class="nav">Cadastro de Produto</a></li>
        <li class="divider"></li>
         <li><a href="redir_editorFunc.jsp" class="nav">Editar Funcionários</a></li>
        <li class="divider"></li>
         <li><a href="redir_relatorios.jsp" class="nav">Relatórios</a></li>
        <li class="divider"></li>      
        
        <li><a href="#" class="nav">Ajuda</a></li>
        <li class="divider"></li>
      </ul>
   